...
for kkk = 1:4
    ...
    hx =            1/8/2^kkk; % 网格size h,逐次加密 check convergence rates
    hy =                   hx/2; % 网格size h,逐次加密 check convergence rates    
    xh =               0:hx:1; % x-方向节点
    yh =               0:hy:1; % y-方向节点
    ...
    ibd_Db =                2:(Nx-1); % 为了
    ibd_Dl = (Nx+1):Nx:(itot-2*Nx+1); % 使得
    ibd_Dt =    (itot-Nx+2):(itot-1); % 矩阵
    ibd_Dr =     (2*Nx):Nx:(itot-Nx); % 对称
    % 对称化处理系数矩阵,先上下边界
...
% 定义 exact solution 真实解
function y = f_u(x1,x2)
    y = exp(4*x1) + cos(4*pi*x2);
end

% 定义 boundary data 边界数据
function y = f_g(x1,x2)
    y = exp(4*x1) + cos(4*pi*x2);
end
